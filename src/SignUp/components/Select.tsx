...
const Select: React.FC<Props> = props => {
    ...
    return (
        <Component focus={`${focus}`} loading={`${props.loading}`} onClick={() => setFocus(!focus)} selected={city.id === 0 ? `${false}` : `${true}`}>
            <label>{props.placeholder}</label>
            <span>{city.name}</span>
            <input type='text' disabled></input>
            <List focus={`${focus}`}>
                {props.options.map(item => item.id !== 0 ? <ListItem focus={`${focus}`} onClick={() => {setCity(item); props.onInput(item.id)}}>{item.name}</ListItem> : '')}
            </List>
        </Component>
    )
}
...
const Component = styled.div<{loading: string, selected: string, focus: string}>`
    width: 17rem;
    margin-top: 1rem;
    position: relative;
    label {
